package com.greenbank.api.database.sql.h2;
...
public class H2DatabaseConnect implements DatabaseConnector {
    private Connection connection;
    private final DatabaseSettings settings;
    private final Settings configSettings;
    private final String tableName;

    public H2DatabaseConnect(@NotNull IGreenbank plugin) {
    ...
    // org.h2.Driver
    // jdbc:h2:mem:testdb
    // jdbc:h2:file:D:/Downloads/Folder
    ...
    public void createSQLConnection() {
        ...
        config.setDriverClassName(settings.getAdvancedH2DriverClassName());
        config.setJdbcUrl(settings.getH2JdbcUrl());
        config.setUsername(settings.getH2Username());
        config.setPassword(settings.getH2Password());
        config.setMaximumPoolSize(settings.getMaximumPoolSize());
        config.setPoolName(settings.getPoolName());
        CompletableFuture.runAsync(() -> {
            try (HikariDataSource dataSource = new HikariDataSource(config)) {
                connection = dataSource.getConnection();
                try (PreparedStatement statement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS " + tableName + " (uuid varchar(36) NOT NULL, doNotDisturb BOOLEAN, socialSpy BOOLEAN);")) {
